<div class="relative mt-3 md:mt-0" x-data="{ isOpen: true }" @click.away="isOpen = false">
    ...
    <div wire:loading class="absolute top-0 right-0 mr-3 mb-1">
                ...
                <ul>
                    @foreach($searchResults as $result)
                        <li class="border-b border-gray-700">
                            <a 
                                href="{{ route('movies.show', $result['id']) }}" class="flex items-center hover:bg-gray-700 px-3 py-3"
                                @if ($loop->last) 
                                    @keydown.tab="isOpen = false" 
                                @endif
                            >
                            @if ($result['poster_path'])
                                <img src="https://image.tmdb.org/t/p/w92/{{ $result['poster_path'] }}" alt="picture" class="w-8">
                            @else
